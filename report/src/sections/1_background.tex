...
% MSTs provide a natural representation of datasets for clustering, for example, by deleting the longest edges in the MST (i.e. the most distant or dissimilar points) you can partition the graph into k connected components, which can be interpreted as k distinct clusters.
% This approach is particularly effective for identifying clusters of varying densities and shapes, as it \textit{does not rely on strong assumptions about the data distribution, convexity or apriori knowledge like the number of clusters}.
